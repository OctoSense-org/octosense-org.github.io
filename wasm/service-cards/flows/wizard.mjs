...
if (root) {
  try {
    const service = await import(root.dataset.serviceUrl);
    initialize(root, service);
  } catch (error) {
    root.dataset.status = 'error';
    root.querySelector('[data-loading-text]').textContent = root.dataset.locale === 'en' ? 'The service could not load. Please retry.' : '服务未能加载，请重试';
    const retry = root.querySelector('[data-retry]'); retry.hidden = false;
    retry.addEventListener('click', () => location.reload());
    console.error('[OctoSense flow]', error);
  }
}

function initialize(root, {createSession, getView, activateControl, nextUpdate, goBack, restart, errorMessage}) {
  ...
  const strings = en ? {
    ...
    complete:'You’re all set. Review the details, or start again.',
  ...
  async function fetchBundle() {
    const bundleURL=new URL(root.dataset.bundleUrl,location.href);
